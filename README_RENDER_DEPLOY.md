...
| 파이프라인 실행 | 로컬 Python 환경 | 자동 300 종목 제한 |
| 데이터 수집 | 전체 종목 (~2600개) | 자동 제한 (RENDER_COLLECT_LIMIT, 기본 300) |

### Render 디스크 유의사항

Render 무료 플랜은 **휘발성 디스크**를 사용합니다.
서비스 재시작(idle 15분 후, 재배포 시) 마다 `data/`, `models/`, `reports/` 내용이 초기화됩니다.

- 매 재시작 후 **전체 파이프라인**을 다시 실행해야 합니다.
- 데이터 영속성이 필요하면 Render **Persistent Disk** (Starter 플랜, $7/월) 를 추가하세요.

---

## Render에서 파이프라인 첫 실행 가이드

1. 앱 접속 → **AI 후보 리스트** 페이지
2. 파란 Render 안내 배너 확인 (자동 300종목 제한 표시)
3. **"전체 파이프라인 실행"** 클릭 (최초 1회 필요, 약 30~60분 소요)
   - 데이터 수집 → 피처/라벨 생성 → 모델 학습 → 예측 → Top100 → 장중 필터
   - 각 단계 진행 상태가 화면에 표시됨
4. 완료 후 **예산배분 및 주문** 페이지에서 매수 실행 가능

> **이후 재실행은 "빠른 후보 생성 (Render 권장)" 버튼 사용** (2~5분 소요)  
> 서비스 재시작 후에는 전체 파이프라인을 다시 실행해야 합니다.

---

## RENDER_COLLECT_LIMIT 설정

데이터 수집 종목 수를 조절하려면 Render Dashboard → Environment Variables에서 설정:
...
### 파이프라인 데이터 수집이 멈추는 경우

- **원인**: pykrx가 KRX 서버에 HTTP 요청 중 응답 지연
- **해결**: 자동 타임아웃 (종목당 15초) 적용됨. 멈춰도 다음 종목으로 진행
- **확인**: Render Dashboard → Logs 탭에서 `[RENDER]` 로그 확인
...
2. ❌ FAIL 항목 해결 후 재시도
3. `KIS_MOCK_APP_KEY`, `KIS_MOCK_APP_SECRET`, `KIS_MOCK_ACCOUNT_NO` 설정 확인
